...
use crate::controller::MinecraftController;
use async_std::sync::{Arc, Mutex};

use std::fmt;

async fn run_once(controller: &Arc<Mutex<MinecraftController>>, crds: Vec<Minecraft>) {
    for crd in crds {
...
async fn main() -> anyhow::Result<()> {
    ...
    let minecrafts: Api<Minecraft> = Api::namespaced(client.clone(), &namespace);
    let lp = ListParams::default().timeout(20); // low timeout in this example
    let rf = Reflector::new(minecrafts).params(lp);

    let inf: Informer<Minecraft> = Informer::new(Api::namespaced(client.clone(), &namespace));
    ...
    let controller = Arc::new(Mutex::new(MinecraftController::new(
        &namespace,
        client,
        has_openshift,
    )));
